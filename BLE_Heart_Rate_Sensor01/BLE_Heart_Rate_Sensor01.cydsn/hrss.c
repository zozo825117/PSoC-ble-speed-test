...
*
*******************************************************************************/
CYBLE_API_RESULT_T HrssSetBodySensorLocation(CYBLE_HRS_BSL_T location)
{
    return (CyBle_HrssSetCharacteristicValue(CYBLE_HRS_BSL, CYBLE_HRS_BSL_CHAR_LEN, (uint8*)&location));
}
...
*
*******************************************************************************/
CYBLE_API_RESULT_T HrssSendHeartRateNtf()
{
...
void SimulateHeartRate(void)
{
    ...
    static uint16 rrInterval            = SIM_RR_INTERVAL_MIN;
    ...
    rrInterval += SIM_RR_INTERVAL_INCREMENT;
    if(rrInterval > SIM_RR_INTERVAL_MAX)
    {
        rrInterval = SIM_RR_INTERVAL_MIN;
    }
    ...
    apiResult = HrssSendHeartRateNtf();
